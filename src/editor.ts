...
import { validatePython, getRuffWorkspace } from './python_utils'
...
const _activeViews = new Map<EditorView, Compartment>()
 ...
 */
export function unregisterEditor(view: EditorView) {
 ...
 */
export function setEditorTheme(dark: boolean | null) {
    _forcedDark = dark
...
export async function createNewEditor(editorElement: HTMLElement, fn: string, content: string, options: EditorOptions): Promise<EditorView> {
    let mode: any[] = []
    fn = fn.toLowerCase()
    if (fn.endsWith('.py')) {
 ...
 */
export function getEditorFromElement(element: HTMLElement): EditorView | null {
 ...
 */
export function addUpdateHandler(editorView: EditorView, callback: (update: ViewUpdate) => void) {
