...
// This file is bundled as a classic (non-module) service worker script.
...
function normalizeUrl(s: string): URL {
  const url = new URL(s);
  if (url.pathname === '/') {
...
function isAppShell(request: Request, url: URL): boolean {
  ...
  event.respondWith((async () => {
    ...
    if (r) {
      ...
      try {
        ...
        // Cache the precache list plus the hashed build output under
        // /assets/, so the app keeps working offline after the first load.
        const sameOrigin = url.origin === self.location.origin;
        if (contentToCache.has(url.pathname) ||
            (sameOrigin && rsp.ok && url.pathname.startsWith('/assets/'))) {
          log(`Caching: ${url}`);
