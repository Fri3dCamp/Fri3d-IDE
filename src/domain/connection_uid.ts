...
export class ConnectionUID {
    ...
    static uidtr: Record<string, string> = {
        'B':'8',  'E':'F',  'G':'6',  'I':'1',  'L':'1',  'O':'0',  'Q':'0',  'S':'5',
    };

    static random(): ConnectionUID {
    ...
    static parse(input: string): ConnectionUID {
    ...
    static _base24(n: bigint, length: number): string {
        const base = BigInt(24);
        let res = "";
        let prev: string | null = null;
        while (res.length < length) {
            let c = ConnectionUID.uidcs[Number(n % base)];
            if (c === prev) {
                c = ConnectionUID.uidcs[Number((n + BigInt(1)) % base)];
            }
            prev = c;
            res += c;
            n /= base;
