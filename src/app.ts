...
import { addUpdateHandler, createNewEditor, getEditorFromElement, unregisterEditor, setEditorTheme } from './editor'
import { displayOpenFile, createTab } from './editor_tabs'
import { serial as webSerialPolyfill } from 'web-serial-polyfill'
import { WebSerial, WebBluetooth, WebSocketREPL, WebRTCTransport } from './transports'
import { MpRawMode } from './rawmode'
import { getPkgIndexes, rawInstallPkg } from './package_mgr'
import { ConnectionUID } from './connection_uid'
import { translations } from './lang'
import { parseStackTrace, validatePython, disassembleMPY, minifyPython, prettifyPython } from './python_utils'
import { MicroPythonOSWASM } from './emulator'
...
import { splitPath, sleep, fetchJSON, getCssPropertyValue,
         QSA as _QSA, QS as _QS, QID as _QID, iOS, sanitizeHTML, escapeHtml,
         sizeFmt, indicateActivity, setupTabs, report,
         withLoader, showLoader } from './utils'

// Loosen the selector helpers' types during the TS migration: this file leans
// on dynamic DOM access (.checked, .value, dataset, …) too heavily to benefit
// from strict element types. Runtime behavior is identical.
const QSA = _QSA as (sel: string) => any[]
const QS = _QS as (sel: string) => any
...
function saveUiSettings(settings: any) {
...
let editor: any, term: Terminal, port: any
let editorFn = ''
...
function writeTerminal(data: any) {
    term.write(data)
...
export async function connectDevice(type: string) {
    if (port) {
    ...
    if (QID('interrupt-device').checked) {
        ...
        try {
            ...
            let fs_stats: any = [null, null, null];
            try {
            ...
            if        (fs_tree.filter((x: any) => x.path === '/main.py').length) {
                await _raw_loadFile(raw, '/main.py')
            } else if (fs_tree.filter((x: any) => x.path === '/code.py').length) {
...
function setupModalA11y(dialog: any, label: any) {
    dialog.setAttribute('role', 'dialog')
    dialog.setAttribute('aria-modal', 'true')
    if (label) dialog.setAttribute('aria-label', label)
    const prevFocus = document.activeElement as any
...
function showPromptDialog(message: string, { value = '', placeholder = '', password = false } = {}) {
...
function showCreateItemDialog(parentPath: string) {
    return new Promise<{ name: string, isFolder: boolean } | null>((resolve) => {
        ...
        let isFolder = false
        function setType(folder: boolean) {
            isFolder = folder
        ...
        function close(result: { name: string, isFolder: boolean } | null) {
            backdrop.remove()
            restoreFocus()
            resolve(result)
        }
...
function showAppWizardDialog() {
    return new Promise<any>((resolve) => {
        ...
        const fullnameInput = dialog.querySelector('#fri3d-app-fullname') as any
        const nameInput = dialog.querySelector('#fri3d-app-name') as any
        const errorEl = dialog.querySelector('#fri3d-app-error') as any

        const restoreFocus = setupModalA11y(dialog, T('app.dialog.title', 'Create New App'))

        const iconCanvas = dialog.querySelector('#fri3d-app-icon') as any
        const iconCtx = iconCanvas.getContext('2d')
        const eraserBtn = dialog.querySelector('#fri3d-icon-eraser') as any
        const colorInput = dialog.querySelector('#fri3d-icon-color') as any
        const sizeSelect = dialog.querySelector('#fri3d-icon-size') as any
        ...
        let strokeLast: any = null
        function iconPos(e: any) {
        ...
        iconCanvas.addEventListener('pointerdown', (e: any) => {
            e.preventDefault()
        ...
        iconCanvas.addEventListener('pointermove', (e: any) => {
        ...
        function tryCreate() {
            ...
            resolve({
                ...
                publisher: (dialog.querySelector('#fri3d-app-publisher') as any).value,
                version: (dialog.querySelector('#fri3d-app-version') as any).value,
                description: (dialog.querySelector('#fri3d-app-desc') as any).value,
                longDescription: (dialog.querySelector('#fri3d-app-long-desc') as any).value,
                category: (dialog.querySelector('#fri3d-app-category') as any).value,
                template: (dialog.querySelector('#fri3d-app-template') as any).value,
                iconDataUrl,
            })
        }
...
function renderAutoIcon(ctx: any, appName: string, fullname: string) {
    let hash = 0
...
function validateAppFullname(fullname: string) {
...
function makeMainPyBoilerplate(appName: string, template: string) {
...
async function execReplNoFollow(cmd: string) {
...
function _updateFileTree(fs_tree: any, fs_stats: any)
{
    const [fs_used, _fs_free, fs_size] = fs_stats;

    function sorted(content: any) {
        // Natural sort by name
        if (QID('use-natural-sort').checked) {
            const collator = new Intl.Collator(undefined, { numeric: true, sensitivity: 'base' })
            content.sort((a: any, b: any) => collator.compare(a.name, b.name))
        }

        // Stable-sort folders first
        content.sort((a: any, b: any) => (('content' in a)?0:1) - (('content' in b)?0:1))

        return content
    }

    const changed_files: any[] = []
    QSA("#menu-file-tree .changed").forEach((file) => {
        changed_files.push(file.dataset.fn)
    })
    const open_files: any[] = []
    ...
    if (!fileTree.dataset.delegationBound) {
        ...
        fileTree.addEventListener('click', (e: any) => {
    ...
    function buildTree(node: any, depth: number) {
        ...
        for (const n of sorted(node)) {
            ...
            if ('content' in n) {
                ...
                } else if (['.CRT', '.PEM', '.DER', '.CER', '.PFX', '.P12'].some((x: string) => fnuc.endsWith(x))) {
...
function showFileTreeLoader(message = T('files.loading', 'Loading files…')) {
    ...
    return {
        update(newMessage: any) { el.querySelector('.loader-label').textContent = newMessage },
        hide() { el.remove() },
    }
}

async function _raw_updateFileTree(raw: any) {
    let fs_stats: any = [null, null, null];
    try {
        fs_stats = await raw.getFsStats()
    } catch (err) {
...
export function fileTreeSelect(fn: string) {
...
export async function fileClick(fn: string) {
    if (!port) return;
...
async function _loadContent(fn: string, content: any, editorElement: any) {
    ...
    if (imageMimeType && content instanceof Uint8Array) {
        ...
        addUpdateHandler(editor, (update: any) => {
...
function _setMdToggleButton(fn: string, mode: string) {
...
export async function toggleMarkdownView(fn: string) {
    ...
    if (currentMode === 'view') {
        ...
        _mdRawContent.set(paneEl, content)
        editorEl.innerHTML = `<div class="marked-viewer">` + DOMPurify.sanitize(marked(content) as string) + `</div>`
        paneEl.dataset.mdMode = 'view'
        if (isActive) {
            editor = null
        }
...
export async function loadAllPkgIndexes() {
    ...
    if (!pkgList.dataset.delegationBound) {
        ...
        pkgList.addEventListener('click', (e: any) => {
        ...
        for (const i of await getPkgIndexes()) {
            ...
            for (const pkg of i.index.packages) {
                ...
                if (pkg.name.includes('-')) {
                    ...
                    const exists = i.index.packages.some((pkg: any) => (pkg.name === parent))
                    if (exists) {
                        offset = '&emsp;'
                    }
                }
                const keywords = pkg.keywords ? pkg.keywords.split(',').map((x: any) => x.trim()) : [];
...
async function _raw_installPkg(raw: any, pkg: any, { version=null }: any = {}) {
...
export async function installPkg(pkg: any, { version=null }: any = {}) {
    if (!port) {
        toastr.info(T('app.connect-first', 'Connect your board first'))
        return
    }
...
;(function initSideMenuResizer() {
    ...
    resizer.addEventListener('mousedown', (event: any) => {
...
function hexViewer(arrayBuffer: any, targetElement: any) {
    ...
    function toPrintableAscii(n: number) {
...
(async () => {
    ...
    lang_sel.addEventListener('change', async function(this: any) {
        await i18next.changeLanguage(this.value)
        applyTranslation()
    })

    const zoom_sel = QID('zoom')
    const applyZoom = (zoomValue: any) => {
        const size = 14 * parseFloat(zoomValue)
        document.documentElement.style.setProperty('--font-size', (size).toFixed(1) + 'px')
        if (term) {
            term.options.fontSize = (size * 0.9).toFixed(1) as unknown as number
        }
    }
    zoom_sel.addEventListener('change', async function(this: any) {
...
`
    ...
    term = new Terminal({
        ...
        fontSize: (14 * 0.9).toFixed(1) as unknown as number,
        theme: xtermTheme,
        cursorBlink: true,
        convertEol: true,
    ...
    function applyColorTheme(pref: any) {
    ...
    QID('color-theme').addEventListener('change', function(this: any) {
    ...
    document.addEventListener("tabActivated", (event: any) => {
    ...
    if (typeof window.webrepl_url !== 'undefined') {
        await sleep(100)
...
let startY: any, startHeight: any

export function initDrag(e: any) {
