...
const getCssPropertyValue = (name: string) => getComputedStyle(document.documentElement).getPropertyValue(name)

const QSA = (x: string) => [...document.querySelectorAll(x)]
...
export function setupTabs(containerNode: ParentNode) {
...
export function report(title: string, err: Error) {
...
window.addEventListener('error', (e) => {
    if (e instanceof ErrorEvent && e.message.includes('ResizeObserver')) {
        ...
        report("Error", e as unknown as Error)
    }
});
...
let _loaderContainer: HTMLDivElement | null = null
let _loaderSeq = 0
...
function _ensureLoaderContainer(): HTMLDivElement {
...
function _hideLoaderItem(item: HTMLElement) {
...
export function showLoader(message: string) {
    ...
    return {
        id,
        update(newMessage: string) { label.textContent = newMessage },
...
export async function withLoader<T>(message: string, task: (loader: ReturnType<typeof showLoader>) => T | Promise<T>): Promise<T> {
