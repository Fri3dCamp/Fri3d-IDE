...
import { QID } from '../../utils'
import { collectAssistantContext } from '../context/collector'
import type { AssistantRuntime } from '../context/collector'
import { fetchGroundingContext } from '../context/docs_mcp'
import type { GroundingResult } from '../context/docs_mcp'
import { runOpenAICompatibleChat } from '../providers/openai_compatible'
import { getChromePromptApiStatus, runChromePromptApiChat } from '../providers/chrome_prompt_api'
import { getWebLLMStatus, runWebLLMChat, clearWebLLMCache } from '../providers/webllm'
import { getMicroPythonSystemPrompt, getTaskInstruction } from '../prompts/micropython_system_prompt'
...
const DEFAULT_LAYOUT: AssistantLayout = {
    width: 360,
    collapsed: false,
}
...
// Values edited through DOM inputs may become strings at runtime, so the
// settings object stays loosely typed while the defaults provide the shape.
type AssistantSettings = typeof DEFAULTS & Record<string, any>
...
function loadLayout(): AssistantLayout {
    try {
...
function saveLayout(layout: AssistantLayout) {
...
function setProviderStatus(text: string) {
...
function contextToText(context: any): string {
    return JSON.stringify(context, null, 2)
}

function buildMessages(prompt: string, taskId: string, context: any, grounding: GroundingResult) {
    const parts = [
        `User request:\n${prompt}`,
        `Task mode:\n${taskId}`,
...
async function callProvider(provider: string, payload: any, settings: AssistantSettings, onProviderStatus: (status: string) => void): Promise<string> {
...
export function initAssistantPanel(runtime: AssistantRuntime) {
    ...
    const advancedModeEl = QID('advanced-mode') as HTMLInputElement | null
    const providerSelectEl = QID('assistant-provider') as HTMLSelectElement | null
    ...
    for (const [id, key, prop] of mapping) {
        // The mapped controls are <input> and <select> elements; HTMLInputElement
        // covers the `value`/`checked` members used here.
        const el = QID(id) as HTMLInputElement | null
        if (!el) continue
        if (prop === 'checked') {
    ...
    async function runPrompt(prompt: string, taskId: string = 'none') {
    ...
    QID('assistant-ask')!.addEventListener('click', async () => {
        try {
            await runPrompt((QID('assistant-prompt') as HTMLTextAreaElement).value, (QID('assistant-task-preset') as HTMLSelectElement).value)
        } catch (err: any) {
            setStatus('error')
    ...
    QID('assistant-run-task')!.addEventListener('click', async () => {
        const taskId = (QID('assistant-task-preset') as HTMLSelectElement).value
        const extra = TASK_HELP[taskId] || ''
        const joined = [(QID('assistant-prompt') as HTMLTextAreaElement).value.trim(), extra].filter(Boolean).join('\n\n')
        ;(QID('assistant-prompt') as HTMLTextAreaElement).value = joined
        try {
    ...
    QID('assistant-explain-selection')!.addEventListener('click', async () => {
        ...
        ;(QID('assistant-prompt') as HTMLTextAreaElement).value = text
        try {
            await runPrompt(text, (QID('assistant-task-preset') as HTMLSelectElement).value)
        } catch (err: any) {
            setStatus('error')
            toastr.error(err.message || String(err), 'Assistant error')
        }
    ...
    QID('assistant-fix-traceback')!.addEventListener('click', async () => {
        ...
        ;(QID('assistant-prompt') as HTMLTextAreaElement).value = text
        ;(QID('assistant-task-preset') as HTMLSelectElement).value = 'traceback-fix'
        try {
