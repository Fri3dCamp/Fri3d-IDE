...
// Base for the large generated assets (wasm binaries, *_vfs.tar.gz). They are
// placed under public/assets/ by scripts/prepare.mjs, so both the dev server
// and the production build serve local copies.
...
export async function validatePython(filename: string, content: string, devInfo?: any): Promise<ParsedStackTrace | undefined> {
    ...
    try {
        ...
        const result = await compile_v6(fname, content, options as string[], wasmUrlV6)
        if (result.status !== 0) {
...
export async function compilePython(filename: string, content: string | ArrayBuffer, devInfo?: any): Promise<Uint8Array> {
...
let _tools_vm: any;
let _ruff_wspace: RuffWorkspace | undefined;

export async function loadVFS(vm: any, url: string): Promise<void> {
    ...
    for (const entry of tar.fileInfos) {
        ...
        } else if ((entry.type as number) == 48) {
            let data: Blob | ArrayBuffer = await tar.getFileBlob(entry.name)
...
export async function getToolsVM(): Promise<any> {
...
export async function getRuffWorkspace(): Promise<RuffWorkspace | undefined> {
...
export async function minifyPython(buffer: string | Uint8Array): Promise<string> {
...
export async function prettifyPython(buffer: string): Promise<string> {
    const ruff = await getRuffWorkspace()
    return ruff!.format(buffer)
}

export async function disassembleMPY(buffer: Uint8Array | ArrayBuffer): Promise<string> {
    const vm = await getToolsVM()
